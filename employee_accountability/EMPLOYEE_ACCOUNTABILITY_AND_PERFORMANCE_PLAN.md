...
**Global:**
- `admin` → Global system admin (1 user)

**Per Department (Admin + General User):**

| Department | Admin User | General User |
|------------|-----------|---------------|
| Sales | `admin.dept.sales` | `user.dept.sales` |
| Projects/PMO | `admin.dept.projects` | `user.dept.projects` |
| Operations/Compliance | `admin.dept.ops` | `user.dept.ops` |

**Individual Salespeople:**
- `sales.person.X` → 14 individual users for sales reps with direct customer access (for commission tracking)

**Total User Count:**
- 1 Global admin
- 3 Department admins
- 3 Department general users
- 14 Individual salespeople
- **= 21 users** (down from 33)

> All **day-to-day operations in back-office departments** happen under shared users (general or admin based on role).
...
1. Number of **billable Odoo users** significantly reduced (33 → 21, saving ~36% on licenses).
